export const drawFaceAndRect = (descriptions, faceDB, participants, ctx) => {
        ...
        descriptions.forEach((desc) => {
            ...
            if (bestMatch._label != "unknown") {
                let filterParticipants = participants.filter(
                    (participant) => `${participant.name} - ${participant.roll}` == bestMatch._label
                );
                // console.log(filterParticipants);
                bestMatch._label = filterParticipants[0].name + " " + " (" + filterParticipants[0].roll + ") ";
            }
